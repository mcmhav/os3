...
public class Process implements Constants
{
	...
	public void updateStatistics(Statistics statistics) {
		...
		statistics.totNoTimeInRQ += nofTimesInReadyQueue;
		statistics.totNoTimeInIOQ += nofTimesInIoQueue;
		statistics.totTSpentInRQ += timeSpentInReadyQueue;
		statistics.cpuTotT += timeSpentInCpu;
		statistics.totTWaitForIO += timeSpentWaitingForIo;
		statistics.ioTotT += timeSpentInIo;
		statistics.totSysT += (end - start);
	}

//	public void updateFinalStat(Statistics s) {
//		s.totSysT += (end - start);
//	}
	
	/**
	 * returning time til next IO
	 * if its 0 it returns a random number based on avgIO
	 * @return
	 */
	...
	/**
	 * returning cputime needed
	 * @return
	 */
	public long getNeededCpuT() {
	 ...
	 * decrementing time needed
	 * and reducing time til IO
	 * and setting the endtime.
	 * @param c
	 */
	public synchronized void cpuLeft(long c) {
		timeSpentInCpu += c - timeOfLastEvent;
		cpuTimeNeeded -= c - timeOfLastEvent;
		timeToNextIoOperation -= c - timeOfLastEvent;
		timeOfLastEvent = c;
		end = c;
		notifyAll();
	}
	 ...
	 * @param c
	 */
	public synchronized void cpuS(long c) {
	...
	/**
	 * Starting the cpu queue
	 * incrementing times in the ready queue
	 * @param c
	 */
	public synchronized void cpuQS(long c) {
	...
	/**
	 * Starting the I/O queue
	 * updates times its been in the queue
	 * and the time spent waiting
	 * @param c
	 */
	public synchronized void ioQS(long c) {
		nofTimesInIoQueue++;
		timeSpentInReadyQueue += c - timeOfLastEvent;
		timeOfLastEvent = c;
		notifyAll();
	...
	/**
	 * I/O start, updates time waiting for the IO
	 * and updates the time of the last event.
	 * @param c
	 */
	public synchronized void ioS(long c) {
	...
	/**
	 * IO time left synchronized, adding to the totaltime spent
	 * calculating the time to next operation
	 * and the time of the last event
	 * @param c
	 */
	public synchronized void ioLeft(long c) {
